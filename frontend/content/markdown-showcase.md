...
同一个 Markdown 文档源里的页面，推荐按源码文件的相对路径来链接。同步到 Modex 后，`docsctl` 会把存在的 `.md` / `.mdx` 链接改成站内文档地址；页面渲染时也会兜底解析一次。

```md
<!-- 当前文件：docs/runtime/threadpool.md -->

[查看运行参数](./config.md)
[查看故障排查](../troubleshooting/timeout.md)
[跳到配置章节](./config.md#队列参数)
```

也可以直接写 Modex 站内绝对地址：

```md
[构建缓存清理](/docs/CBB/latest/build-cache)
```

<Note>
只有指向已同步 Markdown 文件的相对链接会自动转成站内文档页。外部链接、`#标题`、图片、附件和不存在的 `.md` 文件会保持原样。
</Note>
